...
test('Login to Dice.com and Search and Apply', async ({ browser }) => {
    ...
    await page.fill('input[name="email"]', 'sridathu.sdet@gmail.com');
    await page.click('button[type="submit"]');
    await page.fill('input[name="password"]', 'Friendly@1234');
    await page.waitForTimeout(2000);
    await page.click('button[type="submit"]');

    await page.waitForSelector('input[placeholder="Job title, skill, company, keyword"]', { state: 'visible' });
    await page.waitForTimeout(2000);
    await page.getByRole('combobox', { name: 'Job title, skill, company,' }).fill('QA Automation Engineer');
